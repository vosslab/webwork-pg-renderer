...
- Runtime: user-editable problems under `private/` (mounted as `private/`), logs in `logs/`, config defaults in `render_app.conf.dist` (copy to `render_app.conf` locally).
...
- Smoke (server running): `perl script/smoke.pl` uses `Mojo::UserAgent` to hit `/health` and `POST /` (alias `/render-api` stays supported). `./script/smoke.sh` remains for curl-based checks.
...
- No formal suite; always verify `GET /health` returns JSON and that `POST /render-api` renders `private/myproblem.pg` (or your file in `private/`).
...
- Never commit secrets; rely on `render_app.conf` and env vars (`MOJO_MODE`, `SITE_HOST`, `CORS_ORIGIN`, JWT secrets). Keep writable paths to `private/` and `logs/`.
## Coding Style
