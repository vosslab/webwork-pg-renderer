...
- UI polish: `public/navbar.css` themes the top bar while `public/pg-modern.css` now also emphasises which PG submit control was clicked by persisting the darker `--pg-primary-pressed` color (via the `.btn-clicked` hook) plus an active-state transform so the Submit/Preview buttons respond visually without touching PG logic.
