...
- Put your problem under `local_pg_files/` on your computer.
...
- Open the renderer at `http://localhost:3000`.
...
Files in `local_pg_files/` on your computer appear inside the tool as `private/`.
For example, `private/myproblem.pg` is the same as your local file
`local_pg_files/myproblem.pg`.
